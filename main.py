...
# 🌡️ 기온 변화 그래프
st.subheader(f"🌡️ {selected_region}의 기온 변화")

fig_temp = go.Figure()
fig_temp.add_trace(go.Scatter(x=filtered["일시"], y=filtered["평균기온(°C)"],
                              mode='lines+markers', name='평균기온'))
fig_temp.add_trace(go.Scatter(x=filtered["일시"], y=filtered["최고기온(°C)"],
                              mode='lines+markers', name='최고기온'))
fig_temp.add_trace(go.Scatter(x=filtered["일시"], y=filtered["최저기온(°C)"],
                              mode='lines+markers', name='최저기온'))

fig_temp.update_layout(title=f"{selected_region}의 일별 기온 변화",
                       xaxis_title="날짜", yaxis_title="기온 (°C)",
                       hovermode="x unified")

st.plotly_chart(fig_temp, use_container_width=True)

# 🌧️ 강수량 및 습도 그래프
st.subheader("🌧️ 강수량 및 습도 변화")
...
fig_rain.update_layout(
    title=f"{selected_region}의 강수량 및 습도",
    xaxis_title="날짜",
